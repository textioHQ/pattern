...
def synsets(word, pos=NOUN):
    ...
    try:
        if pos.startswith(NOUN.lower()):  # "NNS" or "nn" will also pass.
            ...
            raise TypeError(
                "part of speech must be NOUN, VERB, ADJECTIVE or ADVERB, not %s" % repr(pos))
...
class Synset(object):
    ...
    def hyponyms(self, recursive=False, depth=None):
        ...
        p = [Synset(p.getTarget())
             for p in self._synset.getPointers(wn.HYPONYM)]
    ...
    def similar(self):
        ...
        p = [Synset(s(p.getTarget()))
             for p in self._synset.getPointers(wn.SIMILAR)]
        p += [Synset(s(p.getTarget()))
              for p in self._synset.getPointers(wn.ALSO_SEE)]
        return p
    ...
    def similarity(self, synset):
        ...
        try:  # Lin semantic distance measure.
            lin = 2.0 * log(lcs(self, synset).ic) / \
                (log(self.ic * synset.ic) or 1)
    ...
    def load(self):
        ...
        for s in open(f):
            if not s.startswith(("#", "\t")):
                ...
                # Several WordNet3 entries may point to the same WordNet2
                # entry.
                if k is not None:
