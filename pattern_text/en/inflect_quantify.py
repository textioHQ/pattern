...
from pattern_text.en.inflect import pluralize, referenced
...
def number(s):
    ...
    if len(s) > 1:
        ...
        # 10**(len("12")+1) = 1000; 12.0 / 1000 => 0.012
    ...
    for j, x in enumerate(s):
        if x in NUMERALS:
            ...
            # encouter two following thousands (e.g. two million vigintillion
            # is one subtotal).
            i *= O[x]
            ...
            # Words that are not in any dicionary may be numbers (e.g. "2.5" =>
            # 2.5).
            try:
...
def numerals(n, round=2):
    ...
    if isinstance(n, basestring):
        if n.isdigit():
            ...
            # If the float is given as a string, extract the length of the
            # fractional part.
            if round is None:
                round = len(n.split(".")[1])
            n = float(n)
    ...
    # Converting the integral part to a long ensures a better accuracy during
    # the recursion.
    i = long(n // 1)
    f = n - i
    ...
    if i in NUMERALS_INVERSE:  # 11 => eleven
        ...
        while i > base:
            ...
            base *= 1000
        while o > len(ORDER) - 1:
            # This occurs for consecutive thousands: million vigintillion.
            s += " " + ORDER[-1]
            o -= len(ORDER) - 1
        s = "%s %s%s" % (
            numerals(i // (base / 1000)), (o > 1 and ORDER[o - 1] or ""), s)
        r = i % (base / 1000)
    if f != 0:
        ...
        f, z = zshift(f)
        f = f and " %s%s %s" % (
            RADIX, " %s" % ZERO * z, numerals(long(f))) or ""
    else:
        f = ""
    if r == 0:
        return s + f
    elif r >= 1000:
        # Separate hundreds and thousands with a comma: two million, three
        # hundred thousand.
        return "%s%s %s" % (s, THOUSANDS, numerals(r) + f)
    elif r <= 100:
        # Separate hundreds and tens with "and": two thousand three hundred and
        # five.
...
def approximate(word, amount=1, plural={}):
    ...
    try:
        ...
        raise TypeError("can't pluralize %s (not a string)" %
                        word.__class__.__name__)
    # Anything up to 200.
    if amount == 0:
        return "%s %s" % (NONE, p)
    if amount == 1:
    ...
    h = hundreds == 2 and "hundreds of " or (
        hundreds == 1 and "tens of " or "")
    ...
    if len(args) == 1 and isinstance(args[0], (list, tuple)):
        ...
        for word in args[0]:
            try:
                ...
                raise TypeError(
                    "can't count %s (not a string)" % word.__class__.__name__)
...
def reflect(object, quantify=True, replace=readable_types):
    ...
    types = []
    # Classes and modules with a __dict__ attribute listing methods, functions
    # etc.
