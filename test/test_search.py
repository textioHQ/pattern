...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_product(self):
        ...
        # No possibilities.
        self.assertEqual(list(search.product([], repeat=2)), [])
        # One possibility: the empty set.
        self.assertEqual(list(search.product([1], repeat=0)), [()])
...
class TestTaxonomy(unittest.TestCase):
    ...
    def test_taxonomy(self):
        ...
        self.assertEqual(
            t.parents("John Cleese"), ["basil fawlty", "sir lancelot"])
    ...
    def test_classifier(self):
        ...
        c1 = search.Classifier(
            parents=lambda word, chunk=None: word.endswith("ness") and ["quality"] or [])
        c2 = search.Classifier(
            parents=lambda word, chunk=None: chunk == "VP" and ["action"] or [])
        t = search.Taxonomy()
...
class TestConstraint(unittest.TestCase):
    ...
    def _test_constraint(self, constraint, **kwargs):
        ...
        self.assertEqual(
            constraint.taxonomy, kwargs.get("taxonomy", search.taxonomy))
    ...
    def test_fromstring(self):
        ...
        for s, kwargs in (
                ...
                ("(cats+)",
                 dict(words=["cats"], optional=True, multiple=True)),
                ("(cats)+",
                 dict(words=["cats"], optional=True, multiple=True)),
                ("cats+?", dict(words=["cats"], optional=True, multiple=True)),
                ("cats?+", dict(words=["cats"], optional=True, multiple=True)),
                ("^[fat cat]?", dict(
                    words=["fat cat"], first=True, optional=True)),
                ("[^fat cat?]", dict(
                    words=["fat cat"], first=True, optional=True)),
                ("cats\\|dogs", dict(words=["cats|dogs"])),
                ("cats|dogs", dict(words=["cats", "dogs"])),
                ...
                ("\\^cat", dict(words=["^cat"])),
                ("(cat*)+",
                 dict(words=["cat*"], optional=True, multiple=True)),
                ("^black_cat+",
                    dict(words=["black cat"], multiple=True, first=True)),
    ...
    def test_match(self):
        ...
        for constraint, tests in (
                (R("cat|dog"),  [
                 (W("cat"), 1), (W("dog"), 1), (W("fish"), 0)]),
                (R("cat*"),     [(W("cats"), 1)]),
                (R("*cat"),     [(W("tomcat"), 1)]),
                (R("c*t|d*g"),
                    [(W("cat"), 1), (W("cut"), 1), (W("dog"), 1), (W("dig"), 1)]),
                (R("cats|NN*"), [(W("cats", "NNS"), 1), (W("cats"), 0)]),
                (R("^cat"),     [
                    (W("cat", "NN", index=0), 1), (W("cat", "NN", index=1), 0)]),
                (R("*|!cat"),
                 [(W("cat"), 0), (W("dog"), 1), (W("fish"), 1)]),
                (R("my cat"),   [(W("cat"), 0)]),
                # "my cat" is an overspecification of "cat"
                (R("my cat"),   [(S("my cat").words[1], 1)]),
    ...
    def test_string(self):
        ...
        v.first = True
        self.assertEqual(
            v.string, "^[Steven\\*|NN*|SBJ|\(ASSOCIATE\)_PROFESSOR|!bird]+")
...
class TestPattern(unittest.TestCase):
    ...
    def test_fromstring(self):
        ...
        # Assert messy syntax (fix brackets and whitespace, don't fix empty
        # options).
    ...
    def test_match(self):
        ...
        for i, (pattern, test, match) in enumerate((
            ...
            (P("rabbit*"),
             "big white rabbits",    "rabbits"),  # 3
            (P("JJ|NN"),              S("big white rabbits"),   "big"),  # 4
            # 5
            (P("JJ+"),                S("big white rabbits"),   "big white"),
            (P("JJ+ NN*"),            S("big white rabbits"),
                "big white rabbits"),  # 6
            (P("JJ black|white NN*"), S("big white rabbits"),
                "big white rabbits"),  # 7
            (P("NP"),                 S("big white rabbit"),
                "big white rabbit"),  # 8
            # 9 strict
            (P("big? rabbit", X),     S("big white rabbit"),    "rabbit"),
            # 10 explicit
            (P("big? rabbit|NN"),     S("big white rabbit"),    "rabbit"),
            # 11 greedy
            (P("big? rabbit"),
                S("big white rabbit"),    "big white rabbit"),
            (P("rabbit VP JJ"),       S("the rabbit was huge"),
                "the rabbit was huge"),  # 12
            (P("rabbit be JJ"),       S("the rabbit was huge"),
                "the rabbit was huge"),  # 13 lemma
            (P("rabbit be JJ", X),    S("the rabbit was huge"),
                "rabbit was huge"),     # 14
            # 15
            (P("rabbit is JJ"),       S("the rabbit was huge"), None),
            # 16 overlap
            (P("the NP"),
                S("the rabid rodents"),   "the rabid rodents"),
            (P("t*|r*+"),             S("the rabid rodents"),
                "the rabid rodents"),   # 17
            (P("(DT) JJ? NN*"),       S("the rabid rodents"),
                "the rabid rodents"),   # 18
            (P("(DT) JJ? NN*"),       S("the rabbit"),
                "the rabbit"),          # 19
            # 20 greedy
            (P("rabbit"),
                S("the big rabbit"),      "the big rabbit"),
            (P("eat carrot"),         S("is eating a carrot"),
                "is eating a carrot"),  # 21
            (P("eat carrot|NP"),      S("is eating a carrot"),
                "is eating a carrot"),  # 22
            (P("eat NP"),             S("is eating a carrot"),
                "is eating a carrot"),  # 23
            # 24
            (P("eat a"),              S("is eating a carrot"),  "is eating a"),
            (P("!NP carrot"),         S("is eating a carrot"),
                "is eating a carrot"),  # 25
            (P("eat !pizza"),         S("is eating a carrot"),
                "is eating a carrot"),  # 26
            # 27
            (P("eating a"),           S("is eating a carrot"),  "is eating a"),
            # 28
            (P("eating !carrot", X),  S("is eating a carrot"),  "eating a"),
            # 28 NP chunk is a carrot
            (P("eat !carrot"),        S("is eating a carrot"),  None),
            # 30 eat followed by DT
            (P("eat !DT"),            S("is eating a carrot"),  None),
            # 31 a/DT is not NN
            (P("eat !NN"),            S("is eating a carrot"),  "is eating a"),
            # 32 is eating == eat != is
            (P("!be carrot"),
                S("is eating a carrot"),  "is eating a carrot"),
            # 33 VP chunk == eat
            (P("!eat|VP carrot"),     S("is eating a carrot"),  None),
            # 34
            (P("white_rabbit"),       S("big white rabbit"),    None),
            # 35
            (P("[white rabbit]"),     S("big white rabbit"),    None),
            (P("[* white rabbit]"),   S("big white rabbit"),
                "big white rabbit"),    # 36
            (P("[big * rabbit]"),     S("big white rabbit"),
                "big white rabbit"),    # 37
            (P("big [big * rabbit]"), S("big white rabbit"),
                "big white rabbit"),    # 38
            # 39 bad pattern: "+" is literal
            (P("[*+ rabbit]"),        S("big white rabbit"),    None),
        )):
        ...
        g = lambda chunk, constraint: len(
            [w for w in chunk if not constraint.match(w)]) == 0
        # a rabbit != white
        self.assertEqual(P("!white").match(s).string, "the big white rabbit")
        # a white rabbit == white
        self.assertEqual(P("!white", greedy=g).match(s), None)
        ...
        # Assert None, the syntax cannot handle taxonomy items that span
        # multiple chunks.
    ...
    def test_search(self):
        ...
        # Assert all variations are matched (sentence starts with a NN* which
        # must be caught).
        v = search.Pattern.fromstring("(DT) JJ?+ NN*")
        v = v.search(
            Sentence(parse("dogs, black cats and a big white rabbit")))
    ...
    def test_convergence(self):
        ...
        w = ("big", "white", "rabbit", "black", "cats", "is",
             "was", "going", "to", "sleep", "sleepy", "very", "or")
        x = ("DT?", "JJ?+", "NN*", "VP?", "cat", "[*]")
        for i in range(100):
...
class TestMatch(unittest.TestCase):
    ...
    def test_match(self):
        ...
        s = Sentence(
            parse("Death awaits you all with nasty, big, pointy teeth."))
        ...
        # Assert contraint "NN*" links to "Death" and "teeth", and "JJ" to
        # "pointy".
        ...
        # Assert Match.constituents() by constraint, constraint index and list
        # of indices.
        ...
        self.assertEqual(
            m[1].constituents(constraint=(0, 1)), [s.words[-3], s.words[-2]])
        # Assert Match.string.
